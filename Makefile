...
install:
	vim -N -E -c':so %' -c':q!' $(PLUGIN)-$(VERSION).vmb

uninstall:
	vim -N -E -c':RmVimball' -c':q!' $(PLUGIN)-$(VERSION).vmb
...
$(PLUGIN).vmb:
	...
	vim -N -E -c 'ru! vimballPlugin.vim' -c ':call append("0", [ "$(SCRIPT)", "$(AUTOL)", "$(DOC)"])' -c '$$d' -c ":%MkVimball $(PLUGIN)-$(VERSION)  ." -c':q!'
	ln -f $(PLUGIN)-$(VERSION).vmb $(PLUGIN).vmb
     
release: version all

version:
	perl -i.orig -pne 's/(let s:debug)\s*=\s*1/$$1 = 0/' ${AUTOL}
